...
function Services() {
  ...
  const deletecategory = async (id) => {
    axios({
      method: "post",
      url: "https://api.vijnanacademy.com/api/userapp/deleteservices/" + id,
    })
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <img
            src={`https://api.vijnanacademy.com/service/${row.serviceImg}`}
            width="50px"
            height="50px"
          />
        </div>
  ...
  const postsubcategory = async () => {
    let res = await axios.post(
      `https://api.vijnanacademy.com/api/userapp/postappsubcat/`,
      {
        category: category,
      }
    );
